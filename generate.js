...
async function main() {
  ...
  console.log('Compiling view model...');
...
async function getLatestTweets(userId) {
  const response = await twitter.get(`https://api.twitter.com/2/users/${userId}/tweets`, {
    max_results: 10,
...
function getTwitterBearerToken() {
  ...
  if (!token) {
    throw new Error('TWITTER_BEARER_TOKEN environment variable is missing. Provide it.');
  }
  return token;
}
